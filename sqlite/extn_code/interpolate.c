...
static void hexagesimal_func(sqlite3_context *context, int argc, sqlite3_value **argv)
{
  ...
  char *s = sqlite3_mprintf("%s%%d%c%c %%02d\' %%0%d.%df\"", sign == -1 ? "-" : "", 0xc2, 0xb0, decimals + 2 + !!decimals, decimals);
...
static void first_step(sqlite3_context *context, int argc, sqlite3_value **argv)
{
  ...
  if (sqlite3_value_type(argv[0]) != SQLITE_NULL && (!p->init || sqlite3_value_double(argv[1]) < p->timestamp)) {
...
static int interpolate_connect(sqlite3 *db, void *pAux, int argc, const char *const* argv, sqlite3_vtab **ppVtab, char **pzErr)
{
  ...
  *ppVtab = (sqlite3_vtab*)v;
  ...
  for (i = 0; i < v->tablenum; i++) {
    ...
    char *buf = sqlite3_mprintf("SELECT (SELECT %s FROM %s ORDER BY %s LIMIT 1), (SELECT %s FROM %s ORDER BY %s DESC LIMIT 1)",
...
static int check_constraints(interpolate_cursor *c)
{
  ...
  for (i = 0; i < c->constraints_num; i++)
    ...
    case SQLITE_INDEX_CONSTRAINT_EQ: res &= c->timestamp * c->timestamp_order == c->constraints[i].rval;
      break;
    case SQLITE_INDEX_CONSTRAINT_GT: res &= c->timestamp * c->timestamp_order > c->constraints[i].rval;
      break;
    case SQLITE_INDEX_CONSTRAINT_LE: res &= c->timestamp * c->timestamp_order <= c->constraints[i].rval;
      break;
    case SQLITE_INDEX_CONSTRAINT_LT: res &= c->timestamp * c->timestamp_order < c->constraints[i].rval;
      break;
    case SQLITE_INDEX_CONSTRAINT_GE: res &= c->timestamp * c->timestamp_order >= c->constraints[i].rval;
      break;
    }
  return res;
}
...
static int resize_buffer(interpolate_cursor *c)
{
  ...
  do {
	...
	if (c->timestamp_min[i] == DBL_MAX && !isnan(c->context_values[j][i]))
	    c->timestamp_min[i] = sqlite3_column_double(c->stmt, v->timestamp_idx) * c->timestamp_order;
      }
    } else
      if (rc == SQLITE_DONE)
	c->stmt_done = 1;
 ...
 // Internally timestamp must be in ascending order, so restore original value
  ...
  if (ctype != COL_BASE && ctype != COL_ANGLE) {
	...
	sqlite3_free(f);
      ...
      else if (ctype == COL_AVG && !isnan(c->last[q].v) && c->timestamp - c->last[q].t < v->contexts[ccontext] / 4) {
	...
	sqlite3_result_double(ctx, sign * round(precision * (val / (ctype == COL_AVG || ctype == COL_AVG_FILTERED ? v->contexts[ccontext] : 1))) / precision);
      return SQLITE_OK;
    } else {
      /* Min or max */
...
static int interpolate_filter(sqlite3_vtab_cursor *cur, int idxNum, const char *idxStr, int argc, sqlite3_value **argv)
{
  ...
  if (eq || v->subsample > 0) {
	...
	return SQLITE_NOMEM;
      ...
      bufs[j++] = sqlite3_mprintf("SELECT min(%s), max(%s) FROM (\n", v->timestamp, v->timestamp);
      for (i = 0; i < v->tablenum; i++) {
	bufs[j++] = sqlite3_mprintf("  SELECT %s FROM (SELECT %s FROM %s %s)",
				    v->timestamp, v->timestamp, v->tables[i], where);
  ...
  if (eq)
    bufs3[j++] = sqlite3_mprintf("  SELECT %s FROM temp.t%d UNION\n",
				v->timestamp, c->temptable);
  for (i = 0; i < v->tablenum; i++) {
    bufs3[j++] = sqlite3_mprintf("  SELECT %s FROM %s WHERE %s >= %f AND %s <= %f %s",
  ...
  if (k && k != i - 5 && !*where) {
	...
	return rc;
      ...
      if (v->subsample > 0)
	bufs[j++] = sqlite3_mprintf(" GROUP BY %s / %d", v->timestamp, v->subsample);
      bufs[j++] = sqlite3_mprintf(") USING(%s)%s", v->timestamp, i == v->tablenum - 1 ? "" : "\n");
      sqlite3_free(buf);
    }
    bufs[j++] = sqlite3_mprintf(" %s\n", idxStr);
  }
...
static int interpolate_bestindex(sqlite3_vtab *tab, sqlite3_index_info *info)
{
  ...
  if (info->nConstraint) {
	...
	continue;
      ...
      bufs[j++] = sqlite3_mprintf("%s%s %s %%", first ? " AND " : "", v->colnames[info->aConstraint[i].iColumn], op);
      first = 1;
...
int sqlite3_extension_init(
  ...
  for (i = 0; i < sizeof(functions) / sizeof(*functions); i++)
    sqlite3_create_function(db, functions[i].zFunctionName, functions[i].nArg, SQLITE_UTF8,
